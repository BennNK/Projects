...
int main()
{
   ...
   //find, find_if, find_if_not
    ...
    //附：string和vector互相转换。因为有的标准库算法作用在string上不方便，所以将string转化成标准库容器再处理其实不错。
    string str = "abcddea";
    ...
    //copy, copy_if, copy_n
        //可以在同一个容器中进行复制，只要两段的范围不同。
    copy(str4.begin(), str4.end(), str3.begin());//将str4全部范围内的值拷贝并覆盖到str3头部。str3必须必str4大。copy_if就可以加pred限制条件。
    ...
    //sort, stable_sort,is_sorted
        //时间复杂度：sort应用pred NlogN次,stable_sort应用pred Nlog(N^2)次
    str3 = "abcddea, er, ASd , 123, er1,as{():";
    ...
    //binary_search
        //仅能在已 升序 排序后的序列上使用。二分查找时间复杂度为以2为底的log（N）
          //返回bool
