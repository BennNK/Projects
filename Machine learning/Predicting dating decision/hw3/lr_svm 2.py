...
def lr(trainingSet, testSet):
    ...
    for n in range(n_iter):
        ...
        #     for i in range(N):
        #         gradients[j] = gradients[j] + (y_i_hat[i] - y_i[i]) * df_no_label_matrix[i][j]
        #     gradients[j] =  gradients[j] + lamda * w[j]
...
def svm(trainingSet, testSet):
    ...
    for n in range(500):
        ...
        # for i in range(N):
        #      y_i_hat[i] = np.matmul(w,df_no_label_matrix[i])
        ...
        #     for i in range(N):
        #         if y_i[i] * y_i_hat[i] < 1 :
        #             gradients2[j] = gradients2[j] + lamda * w[j] - y_i[i] * df_no_label_matrix[i][j]
        #         else:
        #             gradients2[j] = gradients2[j] + lamda * w[j]
            
        #     gradients2[j] = gradients2[j]/N
            
        # print ('gradients2',gradients2,'\n')
        # print ('gradients2-gradients',gradients2-gradients,'\n')
            
            
        
        descent = step_size * gradients
        
        descent2 = step_size * gradients2
        
        distance = np.linalg.norm(descent)
        
        distance2 = np.linalg.norm(descent2)
